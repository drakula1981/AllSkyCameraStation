...
namespace AllSkyCameraConditionService.Jobs {
   internal class CloudSensor : IJob {
      ...
      public static async Task ReadTemperature() {
         try {
            ...
            Log.Logger.Error($"[CloudSensor] Error while retrieving Cloud Datas : {ioex.Message}");
         } catch (Exception ex) {
            Log.Logger.Error(ex, "[CloudSensor] Error while retrieving Cloud Datas");
         }
