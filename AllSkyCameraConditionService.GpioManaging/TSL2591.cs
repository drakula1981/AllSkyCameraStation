...
internal class TSL2591 : IDisposable {
   ...
   public uint[] GetData() => [I2CRead16(TSL2591_REG_DATA_0), I2CRead16(TSL2591_REG_DATA_1)];
   // Calculate Lux
   ...
   public async Task<double[]?> GetLuxAsync(float temp) {
      uint gain = GainSet;
      ...
      if ((CH0 == 0xFFFF) || (CH1 == 0xFFFF)) return null;
      ...
      return [d0 - d1, d1, d0, Math.Round(integ, 4), again, atime];
   }
   // Write byte
   private void Write8(byte addr, byte cmd) {
      byte[] Command = [(byte)((addr) | TSL2591_CMD), cmd];

      I2C?.Write(Command);
   }
   // Read byte
   private byte I2CRead8(byte addr) {
      byte[] aaddr = [(byte)((addr) | TSL2591_CMD)];
